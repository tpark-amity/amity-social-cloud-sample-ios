...
/**
 ...
 @param audio: Data for the audio file.
 ...
 */
- (nullable EkoObject<EkoMessage *> *)createAudioMessageWithChannelId:(nonnull NSString *)channelId
                                                                audio:(nonnull NSData *)audioData
                                                             fileName:(nonnull NSString *)fileName
                                                             parentId:(nullable NSString *)parentId
                                                                 tags:(nullable NSArray<NSString *> *)tags;

/**
 Downloads the media (File/Audio) attached to this message and returns it.
 
 @discussion Downloading is performed on background thread and after completion results will be provided on the main thread. If no media is found for this message, this method returns nil in completion block. This method will return the local data for those message which sync state is `Syncing` and will download the data from server for those message whose sync state is `Synced` if necessary. Alternatively you can also use `EkoFileRepository` to download the media using the `fileId` present in message.
 
 @param message: EkoMessage instance which contains media
 @param completion: Block to be called once media is fetched
 */
- (void)downloadFileForMessage:(nonnull EkoMessage *)message completion:(void (^_Nonnull)(NSData * _Nullable mediaData, NSError * _Nullable error))completion;

/**
 Downloads the image data attached to this message and returns it.
 
 @discussion Downloading is performed on background thread and after completion results will be provided on the main thread. If no media is found for this message, this method returns nil in completion block. This method will return the local data for those message which sync state is `Syncing` and will download the data from server for those message whose sync state is `Synced` if necessary. Alternatively you can also use `EkoFileRepository` to download the media using the `fileId` present in message.
 
 @param message: EkoMessage instance which contains image
 @param size: EkoMediaSize for image.
 @param completion: Block to be called once media is fetched
 */
- (void)downloadImageForMessage:(nonnull EkoMessage *)message size:(EkoMediaSize)size completion:(void (^_Nonnull)(NSData * _Nullable mediaData, NSError * _Nullable error))completion;

/**
 Cancels on going download of files for given message
 */
- (void)cancelFileDownloadForMessage:(nonnull EkoMessage *)message;

/**
 Cancels on going download of images for given message
 */
- (void)cancelImageDownloadForMessage:(nonnull EkoMessage *)message size:(EkoMediaSize)size;
