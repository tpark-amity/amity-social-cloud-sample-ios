...
/**
   ...
   @param role: Role filter for membership. If you don't want to filter by role, set nil instead
   @param sortBy: Sort the results by `firstCreated` or `lastCreated`
 */
- (nonnull EkoCollection<EkoChannelMembership *> *)membershipsForFilter:(EkoChannelMembershipFilter)filter sortBy:(EkoSortBy)sortBy role:(nullable NSString *)role;
